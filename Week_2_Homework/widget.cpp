    ...
    : QWidget(parent)
{
    ...
    Read_Game=new QPushButton("读取残局",this);
    Read_Game->setGeometry(930,450,100,40);
    Read_Game->setFlat(1);

    Save_Game=new QPushButton("保存游戏",this);
    Save_Game->setGeometry(930,500,100,40);
    Save_Game->setFlat(1);
    ...
    connect(Read_Game,SIGNAL(clicked(bool)),this,SLOT(Read_Game_Slots()));
    connect(Save_Game,SIGNAL(clicked(bool)),this,SLOT(Save_Game_Slots()));
}
...
void Widget::mousePressEvent(QMouseEvent *event)//鼠标点击事件
{
    ...
    if(Step_Number%2==1){
        QMessageBox::information(this,"对方时间","现在是对方的落子时间");
        return;
    }
...
{//有效的移动棋子和吃子操作
    if(Eat_xt==Eat_x && Eat_yt==Eat_y && Eat_valuet==Eat_value && Eaten_xt==Eaten_x && Eaten_yt==Eaten_y && Eaten_valuet==Eaten_value) {return;}
    ...
    Eat_xt=Eat_x; Eat_yt=Eat_y; Eat_valuet=Eat_value; Eaten_xt=Eaten_x; Eaten_yt=Eaten_y; Eaten_valuet=Eaten_value;
    emit move(Eat_x, Eat_y, Eat_value, Eaten_x, Eaten_y, Eaten_value);
    Initialize_Move_Array();
    Is_Win();
    if(Is_Checkmate()){qDebug()<<"checkmate!";QSound::play(":/back/check.wav");}
}
...
void Widget::Time_Out_Slots()
{
    Step_Time_Number++;
    Sum_Time_Number++;
    if(!Game_Is_Over && Step_Time_Number == 30 && Step_Number%2==0) {QMessageBox::information(this,"超时","您当前已经超时，系统判定对方为赢家！");Game_Is_Over=true;return;}
    if(!Game_Is_Over && Step_Time_Number == 30 && Step_Number%2==1) {QMessageBox::information(this,"超时","对方当前已经超时，系统判定您为赢家！");Game_Is_Over=true;return;}
...
void Widget::paintEvent(QPaintEvent *)
{
    ...
    painter.drawPixmap(-10,0,920,1000,QPixmap(":/back/back"));
    for(int i=0;i<9;i++)
    {//画出所有棋子的位置
        for(int j=0;j<10;j++)
        {
            ...
            {
            case 1:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/BK.GIF"));  break;
            case 2:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/BA.GIF"));  break;
            case 3:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/BB.GIF"));  break;
            case 4:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/BN.GIF"));  break;
            case 5:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/BR.GIF"));  break;
            case 6:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/BC.GIF"));  break;
            case 7:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/BP.GIF"));  break;
            case -1:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/RK.GIF"));  break;
            case -2:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/RA.GIF"));  break;
            case -3:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/RB.GIF"));  break;
            case -4:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/RN.GIF"));  break;
            case -5:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/RR.GIF"));  break;
            case -6:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/RC.GIF"));  break;
            case -7:painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/RP.GIF"));  break;
            default:       break;
            }
            if(To_Move[i][j]==10)
            {//画出所有可以移动棋子的位置
                painter.drawImage(QRect(10+100*i,10+100*j,80,80),QImage(":/Chessman/slected.png"));
            }
        }
    }//画出对方上次移动的位置
    painter.drawImage(QRect(10+100*Clicked_X,10+100*Clicked_Y,80,80),QImage(":/Chessman/slected.png"));
}

bool Widget::Is_Checkmate(){
        for(int i=0; i<9; i++){
            for(int j=0; j<10; j++){
                if(Chessman_Coordinate[i][j]<0) {
                    Check_Checkmate(i,j,Chessman_Coordinate[i][j]);
                    for(int x=0; x<9; x++){
                        for(int y=0; y<10;y++){
                            if(To_Move_Checkmate[x][y]==10 && Chessman_Coordinate[x][y]==1) {
                                for(int z=0; z<9; z++) {for(int k=0; k<10; k++){To_Move_Checkmate[z][k]=0;}}
                                return true;
                            }
                        }
                    }
...
void Widget::Check_Checkmate(int i,int j,int value){
    bool Is_Skip=false;//记录炮是否跳过
        switch (value) {
        case 1://将的移动规则
            if(i-1>=3&&Chessman_Coordinate[i-1][j]<=0) To_Move_Checkmate[i-1][j]=10;
            if(i+1<=5&&Chessman_Coordinate[i+1][j]<=0) To_Move_Checkmate[i+1][j]=10;
            if(j-1>=0&&Chessman_Coordinate[i][j+1]<=0) To_Move_Checkmate[i][j-1]=10;
            if(j+1<=2&&Chessman_Coordinate[i][j+1]<=0) To_Move_Checkmate[i][j+1]=10;
            break;
        case 2://士的移动规则
            if(i-1>=3&&j-1>=0&&Chessman_Coordinate[i-1][j-1]<=0) To_Move_Checkmate[i-1][j-1]=10;
            if(i-1>=3&&j+1<=2&&Chessman_Coordinate[i-1][j+1]<=0) To_Move_Checkmate[i-1][j+1]=10;
            if(i+1<=5&&j-1>=0&&Chessman_Coordinate[i+1][j-1]<=0) To_Move_Checkmate[i+1][j-1]=10;
            if(i+1<=5&&j+1<=2&&Chessman_Coordinate[i+1][j+1]<=0) To_Move_Checkmate[i+1][j+1]=10;
            break;
        case 3://相的移动规则
            if(i-2>=0&&j-2>=0&&Chessman_Coordinate[i-2][j-2]<=0) To_Move_Checkmate[i-2][j-2]=10;
            if(i-2>=0&&j+2<=4&&Chessman_Coordinate[i-2][j+2]<=0) To_Move_Checkmate[i-2][j+2]=10;
            if(i+2<=8&&j-2>=0&&Chessman_Coordinate[i+2][j-2]<=0) To_Move_Checkmate[i+2][j-2]=10;
            if(i+2<=8&&j+2<=4&&Chessman_Coordinate[i+2][j+2]<=0) To_Move_Checkmate[i+2][j+2]=10;
            break;
        case 4://马的移动规则，蹩脚马不能移动
            if(i-2>=0&&j-1>=0&&Chessman_Coordinate[i-1][j]==0&&Chessman_Coordinate[i-2][j-1]<=0) To_Move_Checkmate[i-2][j-1]=10;
            if(i-1>=0&&j-2>=0&&Chessman_Coordinate[i][j-1]==0&&Chessman_Coordinate[i-1][j-2]<=0) To_Move_Checkmate[i-1][j-2]=10;
            if(i-2>=0&&j+1<=9&&Chessman_Coordinate[i-1][j]==0&&Chessman_Coordinate[i-2][j+1]<=0) To_Move_Checkmate[i-2][j+1]=10;
            if(i-1>=0&&j+2<=9&&Chessman_Coordinate[i][j+1]==0&&Chessman_Coordinate[i-1][j+2]<=0) To_Move_Checkmate[i-1][j+2]=10;
            if(i+2<=8&&j-1>=0&&Chessman_Coordinate[i+1][j]==0&&Chessman_Coordinate[i+2][j-1]<=0) To_Move_Checkmate[i+2][j-1]=10;
            if(i+1<=8&&j-2>=0&&Chessman_Coordinate[i][j-1]==0&&Chessman_Coordinate[i+1][j-2]<=0) To_Move_Checkmate[i+1][j-2]=10;
            if(i+2<=8&&j+1<=9&&Chessman_Coordinate[i+1][j]==0&&Chessman_Coordinate[i+2][j+1]<=0) To_Move_Checkmate[i+2][j+1]=10;
            if(i+1<=8&&j+2<=9&&Chessman_Coordinate[i][j+1]==0&&Chessman_Coordinate[i+1][j+2]<=0) To_Move_Checkmate[i+1][j+2]=10;
            break;
        case 5://車的移动规则
            for(int x=1;x<=8;x++)
            {//这是車在向右方向可以选择移动的代码，赋值为10，代表该点可以移动
                if(i+x<=8&&Chessman_Coordinate[i+x][j]==0) To_Move_Checkmate[i+x][j]=10;
                else  if(i+x<=8&&Chessman_Coordinate[i+x][j]<0)
                {//在向右方向碰到另一方棋子，该点可以移动，之后跳出循环
                    To_Move_Checkmate[i+x][j]=10;
                    break;
                }
                else  break;
            }
            for(int x=1;x<=8;x++)
            {
                if(i-x>=0&&Chessman_Coordinate[i-x][j]==0) To_Move_Checkmate[i-x][j]=10;
                else  if(i-x>=0&&Chessman_Coordinate[i-x][j]<0)
                {
                    To_Move_Checkmate[i-x][j]=10;
                    break;
            ...
            for(int x=1;x<=9;x++)
            {
                if(j+x<=9&&Chessman_Coordinate[i][j+x]==0) To_Move_Checkmate[i][j+x]=10;
                else  if(j+x<=9&&Chessman_Coordinate[i][j+x]<0)
                {
                    To_Move_Checkmate[i][j+x]=10;
                    break;
                }
                else  break;
            }
            for(int x=1;x<=9;x++)
            {
                if(j-x>=0&&Chessman_Coordinate[i][j-x]==0) To_Move_Checkmate[i][j-x]=10;
                else  if(j-x>=0&&Chessman_Coordinate[i][j-x]<0)
                {
                    To_Move_Checkmate[i][j-x]=10;
                    break;
        ...
        case 6://炮的移动规则
            ...
            for(int x=1;x<=8;x++)
            {
                if(i+x<=8&&Chessman_Coordinate[i+x][j]==0&&!Is_Skip) To_Move_Checkmate[i+x][j]=10;
                else  if(i+x<=8&&Chessman_Coordinate[i+x][j]!=0&&!Is_Skip)
                {//如果碰到棋子，设置Is_Skip为真
                    Is_Skip=true;
                }
            ...
            for(int x=1;x<=8;x++)
            {
                if(i-x>=0&&Chessman_Coordinate[i-x][j]==0&&!Is_Skip) To_Move_Checkmate[i-x][j]=10;
                else  if(i-x>=0&&Chessman_Coordinate[i-x][j]!=0&&!Is_Skip)
                {
                    Is_Skip=true;
                }
            ...
            for(int x=1;x<=9;x++)
            {
                if(j+x<=9&&Chessman_Coordinate[i][j+x]==0&&!Is_Skip) To_Move_Checkmate[i][j+x]=10;
                else  if(j+x<=9&&Chessman_Coordinate[i][j+x]!=0&&!Is_Skip)
                {
                    Is_Skip=true;
                }
            ...
            for(int x=1;x<=9;x++)
            {
                if(j-x>=0&&Chessman_Coordinate[i][j-x]==0&&!Is_Skip) To_Move_Checkmate[i][j-x]=10;
                else  if(j-x>=0&&Chessman_Coordinate[i][j-x]!=0&&!Is_Skip)
                {
                    Is_Skip=true;
                }
        ...
        case 7://兵的移动规则
            if(j<5)//未过河只能向前走
            {
                To_Move_Checkmate[i][j+1]=10;
            }
            else
            {
                if(i-1>=0&&Chessman_Coordinate[i-1][j]<=0) To_Move_Checkmate[i-1][j]=10;
                if(i+1<=8&&Chessman_Coordinate[i+1][j]<=0) To_Move_Checkmate[i+1][j]=10;
                if(j+1<=9&&Chessman_Coordinate[i][j+1]<=0) To_Move_Checkmate[i][j+1]=10;
            }
            break;
        case -1:
            if(i-1>=3&&Chessman_Coordinate[i-1][j]>=0) To_Move_Checkmate[i-1][j]=10;
            if(i+1<=5&&Chessman_Coordinate[i+1][j]>=0) To_Move_Checkmate[i+1][j]=10;
            if(j-1>=7&&Chessman_Coordinate[i][j-1]>=0) To_Move_Checkmate[i][j-1]=10;
            if(j+1<=9&&Chessman_Coordinate[i][j+1]>=0) To_Move_Checkmate[i][j+1]=10;
            break;
        case -2:
            if(i-1>=3&&j-1>=7&&Chessman_Coordinate[i-1][j-1]>=0) To_Move_Checkmate[i-1][j-1]=10;
            if(i-1>=3&&j+1<=9&&Chessman_Coordinate[i-1][j+1]>=0) To_Move_Checkmate[i-1][j+1]=10;
            if(i+1<=5&&j-1>=7&&Chessman_Coordinate[i+1][j-1]>=0) To_Move_Checkmate[i+1][j-1]=10;
            if(i+1<=5&&j+1<=9&&Chessman_Coordinate[i+1][j+1]>=0) To_Move_Checkmate[i+1][j+1]=10;
            break;
        case -3:
            if(i-2>=0&&j-2>=5&&Chessman_Coordinate[i-2][j-2]>=0) To_Move_Checkmate[i-2][j-2]=10;
            if(i-2>=0&&j+2<=9&&Chessman_Coordinate[i-2][j+2]>=0) To_Move_Checkmate[i-2][j+2]=10;
            if(i+2<=8&&j-2>=5&&Chessman_Coordinate[i+2][j-2]>=0) To_Move_Checkmate[i+2][j-2]=10;
            if(i+2<=8&&j+2<=9&&Chessman_Coordinate[i+2][j+2]>=0) To_Move_Checkmate[i+2][j+2]=10;
            break;
        case -4:
            if(i-2>=0&&j-1>=0&&Chessman_Coordinate[i-1][j]==0&&Chessman_Coordinate[i-2][j-1]>=0) To_Move_Checkmate[i-2][j-1]=10;
            if(i-1>=0&&j-2>=0&&Chessman_Coordinate[i][j-1]==0&&Chessman_Coordinate[i-1][j-2]>=0) To_Move_Checkmate[i-1][j-2]=10;
...
void Widget::Read_Game_Slots(){
    ...
    Player1_Time->setText("步时: 00:00:00");
    Player2_Time->setText("步时: 00:00:00");
    emit readgame(fileName);
}

void Widget::Save_Game_Slots(){
    /*
    QString fileName = QFileDialog::getSaveFileName(this,"Save File",QDir::currentPath,"Text File(*txt);;Jpg File(*.jpg)");
    if(fileName.isEmpty())
    {
        QMessageBox::information(this,"Error","FileName is NULL");
        return;
    }*/

    QFile *file = new QFile;
    file->setFileName("saved.txt");
    ...
    if(ok)
    {
        ...
        for(int i=0; i<7; i++){
            ...
            for(int x=0; x<9; x++){
                for(int y=0; y<10; y++){
                    if(Chessman_Coordinate[x][9-y]==-(i+1)){
                        num++;
                        location[0][num]=x;location[1][num]=y;
                    }
                }
            }
            out<< num << " ";
            for(int n=0; n<num; n++){
                out << "<" << location[0][n+1] << "," << location[1][n+1] << ">";
            }
            out << endl;
        }
        out << "black" << endl;
        ...
        QMessageBox::information(this,"ok","棋局已经保存");
